/* =========================================================================
   INTERACTIVE BRACKET
   - Dinamik 4/8/16/32 slot
   - “Çift Taraflı” = 16 slot
   - Puan gizleme ayarı
   ========================================================================= */
...
/* ------------------- boş kutu ------------------- */
const blank = ():Match => ({ players:[{seed:0,name:'?'},{seed:0,name:'?'}] });

/* ------------------- matris oluştur ------------------- */
function buildMatrix(names:string[], full16:boolean):Matrix{
    /* slot sayısı = en küçük 2^n ≥ oyuncuAdedi, min 4, max 32   */
    let size:4|8|16|32 = 4;
    const need = full16 ? 16 : Math.max(4, 2**Math.ceil(Math.log2(Math.max(1,names.length))));
    if      (need<=4)  size=4;
    ...
    /* round-0 */
    const r0:[Match, ...Match[]] = [...Array(size/2)].map((_,i)=>({
        players:[
    ...
    /* takip eden round sayısı */
    const rounds:Matrix=[r0];
    let games=size/4;
    while(games>=1){ rounds.push([...Array(games)].map(blank)); games/=2; }
    return rounds;
}

/* ------------------- kazananı üste taşı ------------------- */
function propagate(mat:Matrix):Matrix{
    const m:Matrix=mat.map(rd=>rd.map(x=>({...x,players:x.players.map(p=>({...p}))})));
...
export default memo(function InteractiveBracket(){
    ...
    .hit:hover + .outline{stroke-width:4;filter:drop-shadow(0 0 8px ${palette.glow2})}
                                        ...
                                        {m.players.map((p,idx)=>(
                                            <>
                                                <text key={'n'+idx} className="txt"
                                                      x={x+18} y={mid+(idx?22:-22)}>{p.name}</text>
                                                {settings.showScores && score && (
                                                    <text key={'s'+idx} className="txt" fontSize={14} fill="#fff"
            ...
            {sel&&(
                <MatchModal match={rounds[sel.r][sel.m]}
                            onSave={saveMeta}
                            onClose={()=>setSel(null)}/>
            )}
        </div>
